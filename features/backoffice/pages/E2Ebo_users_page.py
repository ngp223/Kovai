...
class UsersPage_bo(BasePage):
    ...
    ROLE_BTN = (
        By.XPATH,
        "//button[contains(@class,'_secondaryBtn_1qr2a_240')]"
    )

    CREATE_FIRST_USER_BTN = (
        By.XPATH,
        "//button[contains(normalize-space(),'Crear primer usuario')]"
    )

    NEW_USER_BTN = (
        By.XPATH,
        "//button[contains(normalize-space(),'+ Nuevo Usuario')]"
    )
    ...
    PASSWORD_INPUT = (
        By.XPATH,
        "//input[@placeholder='1234' and @pattern='\\d{4,6}']"
    )
