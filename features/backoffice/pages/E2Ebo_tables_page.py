...
class TablesPage_bo(BasePage):
    ...
    def wait_table_created(self):
        WebDriverWait(self.driver, 10).until(EC.visibility_of(self.current_table))

    def move_table(self):
        ActionChains(self.driver).move_to_element(self.current_table).click_and_hold().move_by_offset(-100, -80).release().perform()
        time.sleep(3)

    def delete_table(self):
        self.current_table.find_element(*self.DELETE_BTN).click()
        time.sleep(2)

        try:
            WebDriverWait(self.driver, 3).until(EC.element_to_be_clickable((By.XPATH, "//button[contains(.,'Eliminar')]"))).click()
        except TimeoutException:
            pass
    ...
    def wait_table_deleted(self):
        WebDriverWait(self.driver, 10).until(EC.staleness_of(self.current_table))
