...
class TablesPage_bo(BasePage):
    ...
    def assign_menu(self):
        lista = WebDriverWait(self.driver, 10).until(EC.presence_of_element_located(self.LIST_VIEW_BTN))
        ...
        Select(
            WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(self.MENU_SELECT))).select_by_visible_text(
            "📋 QA(NOBORRAR)")
        ...
        time.sleep(3)
        WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(self.MAP_VIEW_BTN)).click()
        WebDriverWait(self.driver, 10).until(EC.visibility_of_element_located(self.TABLE))
        time.sleep(3)
