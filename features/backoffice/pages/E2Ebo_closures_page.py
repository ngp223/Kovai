...
class ClosuresPage_bo(BasePage):
    CLOSURES_MENU = (By.XPATH,"//a[@href='/closures']" )
    RESTAURANT_SELECT = (By.XPATH,"//select[option[contains(text(),'Tamus')]]")
    EMPLOYEE_SELECT = (By.XPATH,"//select[contains(@class,'_filterSelect_')]")
    TABLE_ROWS = ( By.XPATH,"//tbody/tr")
    ANY_CELL = (By.XPATH,"//tbody/tr/td[1]")

    def _wait_table_loaded(self):
        WebDriverWait(self.driver, 20).until(EC.presence_of_element_located(self.ANY_CELL))

    def _wait_table_refresh(self, old_rows):
        WebDriverWait(self.driver, 20).until(lambda d: len(d.find_elements(*self.TABLE_ROWS)) != old_rows)
    ...
    def select_restaurant(self, restaurant_name):
        ...
        select = Select(self.wait_visible(self.RESTAURANT_SELECT))
    ...
    def select_employee(self, employee_name):
        ...
        select_el.click()
        option_locator = (By.XPATH,f"//option[contains(normalize-space(),'{employee_name}')]")
        option = WebDriverWait(self.driver, 20).until(EC.element_to_be_clickable(option_locator) )
        option.click()
        self._wait_table_refresh(old_rows)

    def closure_exists(self, date_text):

        locator = (By.XPATH,f"//tbody/tr[td[contains(normalize-space(),'{date_text}')]]")
        try:
