...
def step_modify(context):
    timestamp=datetime.now().strftime("%d/%m/%Y %H:%M:%S")
    new_title=f"Problema2 QA {timestamp}"
    new_description=f"Problema2 QA {timestamp}"
    context.support_page.modify_support_ticket(new_title,new_description)
    context.support_title=new_title
