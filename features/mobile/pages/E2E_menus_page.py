...
class MenusPage:
    ...
    PRIMER_ANADIR_PRODUCTO = (AppiumBy.XPATH, '(//android.view.ViewGroup[@content-desc=", Añadir Producto"])[1]')
    SEGUNDO_ANADIR_PRODUCTO = (AppiumBy.XPATH, '(//android.view.ViewGroup[@content-desc=", Añadir Producto"])[2]')
    TERCER_ANADIR_PRODUCTO = (AppiumBy.XPATH, '(//android.view.ViewGroup[@content-desc=", Añadir Producto"])[3]')
    ...
    def scroll_until_visible(self, locator, max_swipes=8):
        ...
        raise Exception(f"No encontrado {locator} después de {max_swipes} scrolls")

    def open_menus(self):
        self.driver.find_element(
            AppiumBy.ANDROID_UIAUTOMATOR,
            'new UiScrollable(new UiSelector().scrollable(true)).scrollIntoView(new UiSelector().text("Menús"))'
        )
    ...
    def delete_menu(self):
        ...
        papelera = self.driver.find_element(
            AppiumBy.XPATH,
            f'//android.widget.TextView[@text="{self.nombre_menu_creado}"]/ancestor::android.view.ViewGroup[1]//*[@content-desc=""]'
        )
